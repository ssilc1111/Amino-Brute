...
authTypeTest = input('Auth type to use?\nuse \'e\' for email\nuse \'p\' for phone number\nchoice: ')
if authTypeTest.lower() == 'e':
    nameEmailOrPhoneNumber = 'Email Address'
    authType = 'email'
elif authTypeTest.lower() == 'p':
    nameEmailOrPhoneNumber = 'Phone Number'
    authType = 'phoneNumber'
else:
    print('Invaild auth type ...')
    exit()
emailToUse = ''
...
while True:
    ...
    if len(emailToUse.replace(' ','')) == 0:
        print('[Input Error] -- > The targets email / phone number cannot be blank !')
    else:
        print('[Info] -- > The target email / phone number to use will be \'%s\''%str(emailToUse))
        break
